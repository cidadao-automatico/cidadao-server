...
object LawProposal {	
	...
	val simple = {
      ...
      get[String]("std_code")
			...
			case id ~ law_author_id ~ law_proposal_id ~ law_type_id ~ law_status_id ~ law_priority_id ~ law_region_id ~ law_url ~ created_at ~ std_code =>
			LawProposal(id,law_author_id,law_proposal_id,law_type_id,law_status_id,law_priority_id,law_region_id,law_url,created_at,std_code)
		}
	}
  ...
  def save(lawPriority: LawPriority, lawAuthor: LawAuthor, lawRegion: LawRegion, lawType: LawType, lawStatus: LawStatus, url: String, createdAt: Date, stdCode: String){
    DB.withConnection{ implicit connection => 
     SQL("""
